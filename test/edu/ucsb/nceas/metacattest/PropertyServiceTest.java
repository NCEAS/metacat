...
public class PropertyServiceTest extends MCTestCase {
    ...
    /**
     * Run an initial test that always passes to check that the test harness is working.
     */
    ...
    /**
     * Tests reading and writing from properties using the PropertyService class 1. read a single
     * property from the properties file that should exist Read a test property from the
     * PropertyService.  The value should be read successfully.  If PropertyNotFoundException is
     * thrown, the test case fails. Also, if the value returned is null, the test case fails.
     * Strictly speaking, it is not a failure for getProperty to return null, but we know that
     * 'test.mcUser' should have a value. 2. read a single property from the main properties that
     * shouldn't exist Expect a PropertyNotFoundException here.  If the exception is not thrown, the
     * test case fails. 3. read group property names from the main properties read an existing group
     * of property names.  If the result set is zero length, the test case fails. 4. read group
     * properties from the main properties Get the property values for the group names retrieved in
     * test case 3.  If the properties map retrieved is empty, the test case fails. 5. write
     * property to main properties Write a property that already exists to the metacat.properties
     * file.  The value is randomly generated.  The value is then retrieved from the
     * metacat.properties file.  If the retrieved value does not match the generated value, or if an
     * exception is thrown, the test case fails. 6. set property and persist to main properties This
     * does the same as test case 5, but instead of using setProperty which sets the property in
     * memory and persists it to file, this case uses setPropertyNoPersist and persistProperties to
     * do the set and persist atomically. 7. try to write property to main properties that doesn't
     * exist Attempt to set a property that doesn't exist in the properties file.  This should throw
     * a PropertyNotFoundException.  Anything else causes the test case to fail. 8. try to write
     * property nonPersistent to main properties that doesn't exist This does the same as test case
     * 7, but uses the setPropertyNoPersist method.
     */
    public void mainPropertiesTest() {
        ...
        try {
            ...
            if (orgList == null || orgList.size() == 0) {
                fail(
                    "Test 3: Empty vector returned when reading property group names "
                        + "'organization.org'");
            }
            ...
            try {
                ...
                if (metacatProps == null || metacatProps.size() == 0) {
                    fail(
                        "Test 4: Empty map returned when reading property group names "
                            + "'organization.org'");
                }
            } catch (PropertyNotFoundException pnfe) {
            ...
            try {
                ...
                debug("Test 7: set property 'test.property.nonexistent' : " + testValue);
                ...
                debug("Test 7: expected failure writing to property:'test.property.nonexistent' : "
                    + pnfe.getMessage());
            }
            ...
            try {
                testValue = "testing" + random.nextInt();
                debug("Test 8: set property 'test.property.nonexistent' : " + testValue);
                ...
                debug("Test 8: expected failure writing to property:'test.property.nonexistent' : "
                    + pnfe.getMessage());
            }
