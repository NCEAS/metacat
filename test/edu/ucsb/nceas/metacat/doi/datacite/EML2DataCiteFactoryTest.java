...
public class EML2DataCiteFactoryTest extends D1NodeServiceTest {
    ...
    private static final String section = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
    private static final String section0 = "<resource xmlns=\"http://datacite.org/schema/kernel-3\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://datacite.org/schema/kernel-3 https://schema.datacite.org/meta/kernel-3.1/metadata.xsd\"><identifier identifierType=\"DOI\">";
    ...
    private static final String section2 = "for use in a workshop titled \"An Analysis of the Relationship between";
    private static final String section3 = "Productivity and Diversity using Experimental Results from the Long-Term";
    private static final String section4 = "Ecological Research Network\" held at NCEAS in September 1996.</title></titles><publisher>My Metacat Node</publisher><publicationYear>2019</publicationYear><subjects><subject xml:lang=\"en\">Old field grassland</subject><subject xml:lang=\"en\">biomass</subject><subject xml:lang=\"en\">productivity</subject><subject xml:lang=\"en\">species-area</subject><subject xml:lang=\"en\">species richness</subject></subjects><language>en</language><resourceType resourceTypeGeneral=\"Dataset\">Dataset/metadata</resourceType><descriptions><description descriptionType=\"Abstract\" xml:lang=\"en\">";
    private static final String section5 = "This data package contains information about the first year of participation in SNAPP synthesis working groups for different institutions";
    private static final String section6 = "The data source is NCEAS administrative database. The R Script compute the yearly statistics and generate the bar chart. The scripts outputs 1 csv file with the yearly stats and one png file wit the plot.";
    private static final String section7 = "</description></descriptions><formats><format>text/xml</format></formats></resource>";
    /**
     * Constructor
     * @param name
     */
    ...
    public void testGenerateMetadata() throws Exception{
        ...
        assertTrue(result.contains(section0+pid.getValue()+section1));
        assertTrue(result.contains(section2));
        assertTrue(result.contains(section3));
        assertTrue(result.contains(section4));
