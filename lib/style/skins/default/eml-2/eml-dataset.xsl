...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
                ...
                <td class="fortyfive_percent">
                    ...
                    <xsl:if test="dataTable|spatialRaster|spatialVector|storedProcedure|view|otherEntity">
                        <xsl:if test="$withEntityLinks='1' or $displaymodule = 'printall'">
                            <table class="{$tabledefaultStyle}">
                                <xsl:call-template name="filesTable"/>
    ...
    <xsl:template name="filesTable">
        <xsl:if test="dataTable or spatialRaster or spatialVector or storedProcedures or view or otherEntity">
            <tr>
                <th colspan="2">
                    <h4><xsl:text>Data Files in Downloaded Package:</xsl:text></h4>
                </th>
            </tr>
        </xsl:if>

        <tr>
            <td class="rowodd"><b>Name</b></td>
            <td class="roweven"><b>Size</b></td>
            <td class="rowodd"><b>Hash</b></td>
        </tr>

        <xsl:for-each select="dataTable">
            <xsl:call-template name="addFileRow">
            </xsl:call-template>
        </xsl:for-each>
    ...
    <xsl:template name="datasetentity">
        <xsl:if test="dataTable or spatialRaster or spatialVector or storedProcedures or view or otherEntity">
            <tr>
                <th colspan="2">
                    <h4><xsl:text>Data File Information:</xsl:text></h4>
                </th>
            </tr>
        </xsl:if>

        <tr>
            <td class="rowodd"><b>Type</b></td>
        </tr>
        ...
        <xsl:choose>
            <xsl:when test="$displaymodule!='printall'">
                <xsl:for-each select="dataTable">
                    <xsl:call-template name="entityRecord">
                        <xsl:with-param name="type">Data Table</xsl:with-param>
                ...
                <xsl:for-each select="spatialRaster">
                    <xsl:call-template name="entityRecord">
                        <xsl:with-param name="type">Spatial Raster</xsl:with-param>
                ...
                <xsl:for-each select="spatialVector">
                    <xsl:call-template name="entityRecord">
                        <xsl:with-param name="type">Spatial Vector</xsl:with-param>
                ...
                <xsl:for-each select="storedProcedure">
                    <xsl:call-template name="entityRecord">
                        <xsl:with-param name="type">Stored Procedure</xsl:with-param>
    ...
    <xsl:template name="addFileRow">
        <tr>
            ...
            <td width="{$secondColWidth}" class="{$secondColStyle}">
                <xsl:for-each select="./physical/size">
            ...
            <td width="{$firstColWidth}" class="{$firstColStyle}">
                ...
                <xsl:for-each select="./physical/authentication">
                    <xsl:value-of select="."/><xsl:text> </xsl:text>
                    <xsl:if test="./@method">
                        (<xsl:text> </xsl:text><xsl:value-of select="./@method"/>)
                    </xsl:if>
                </xsl:for-each>
            </td>
        </tr>
    ...
    <xsl:template name="entityRecord">
        ...
        <tr>
            ...
            <td width="{$firstColWidth}" class="{$firstColStyle}">
                <xsl:choose>
                    <xsl:when test="./physical/">
                        <xsl:variable name="size" select="./physical/size"/>
