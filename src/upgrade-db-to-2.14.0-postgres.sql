...
CREATE TABLE quota_usage_events (
  ...
  subscriber text, -- the subscriber of the quota
  ...
  requestor text, -- the requestor of the qutao usage
   ...
   CONSTRAINT quota_usage_events_uk2 UNIQUE (subscriber, quota_type, instance_id, status)
);
...
CREATE INDEX quota_usage_events_idx6 ON quota_usage_events (subscriber);
