...
public class Eml210SAXHandler extends DBSAXHandler implements AccessControlInterface {
    ...
    private Hashtable<String, AccessSection> topLevelAccessControlMap =
        new Hashtable<String, AccessSection>();
    ...
    private Vector<Hashtable<String, AccessSection>> additionalAccessMapList =
        new Vector<Hashtable<String, AccessSection>>();
    ...
    public Eml210SAXHandler(
        ...
        try {
            ...
            logMetacat.warn(
                "Eml210SAXHandler.Eml210SAXHandler - we can't get object identifier for metacat id "
                    + identifier);
        }
    }
    ...
    private Vector<AccessSection> getAccessSubTreeListFromDB() throws Exception {
        ...
        String sql = "SELECT controllevel, subtreeid, startnodeid, endnodeid "
            + "FROM xml_accesssubtree WHERE docid like ? " + "ORDER BY startnodeid ASC";
        ...
        if (!handleInlineData) {
            ...
            if (localName.equals(INLINE)) {
                ...
                try {
                    ...
                    logMetacat.error(
                        "Could not fing property 'accNumSeparator'. " + "Setting separator to '.': "
                            + pnfe.getMessage());
                }
                // the new file name will look like docid.rev.2
                inlineDataFileName =
                    docidWithoutRev + seperator + revision + seperator + distributionIndex;
                ...
                currentDistributionSection.setDistributionType(
                    DistributionSection.INLINE_DATA_DISTRIBUTION);
            ...
            if (hitTextNode && parentNode != null) {
                ...
                if (needToCheckAccessModule && (processingAdditionalAccess || processingOtherAccess
                    || processingTopLevelAccess)) {
            ...
            if (atFirstElement) {
                ...
                try {
                    ...
                    try {
                        ...
                        logMetacat.error(
                            "SQL error when trying to get rev list for doc " + docid + " : "
                                + sqle.getMessage());
                        throw (new SAXException(
                            "Doc ID " + docid + " was not found and cannot be updated."));
                    }

                    if (revList.size() > 0) {
                        throw (new SAXException("EML210SaxHandler.startElement - Doc ID " + docid
                                                    + " was deleted and cannot be updated."));
                    } else {
                        throw (new SAXException("EML210SaxHandler.startElement - Doc ID " + docid
                    ...
                    throw (new SAXException(
                        "EML210SaxHandler.startElement - error with action " + action + " : "
                            + e.getMessage()));
                }
            }
            ...
            if (processingTopLevelAccess && needToCheckAccessModule) {
                compareElementNameSpaceAttributes(currentUnchangeableAccessModuleNodeStack,
    ...
    private void compareElementNameSpaceAttributes(
        Stack<NodeRecord> unchangeableNodeStack, Hashtable<String, String> nameSpaces,
        Attributes attributes, String localName, String error) throws SAXException {
    ...
    private void compareAccessTextNode(Stack<NodeRecord> nodeStack, StringBuffer text)
        throws SAXException {
        ...
        try {
            ...
            logMetacat.error(
                "Node stack is empty for text data in startElement for doc id " + docid);
        ...
        logMetacat.debug(
            "Eml210SAXHandler.compareAccessTextNode - \n" + "\t access node type from db:       "
                + node.getNodeType() + "\n" + "\t access node data from db:       "
                + node.getNodeData() + "\n" + "\t access node data from document: "
                + text.toString());
        ...
        if (!node.getNodeType().equals("TEXT") || !docAccessData.equals(dbAccessData)) {
            ...
            throw new SAXException(
                UPDATEACCESSERROR + identifier + " [Eml210SAXHandler.compareAccessTextNode]");
        }// if
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        ...
        if (!handleInlineData) {
            ...
            if (hitTextNode) {
                ...
                } else if (currentTag.equals(REFERENCES) && (processingTopLevelAccess
                    || processingAdditionalAccess || processingOtherAccess)) {
                    // get reference
                    data = (textBuffer.toString()).trim();
            ...
            if (currentTag.equals(ALLOW) || currentTag.equals(DENY)) {
                ...
                if (needToCheckAccessModule && processingTopLevelAccess
                    ...
                    logMetacat.error(
                        "Access node stack is not empty after " + "parsing access subtree");
                ...
                // If the current Distribution is inline or data and it doesn't have an access
                // section
                // we use the top level access section (if it exists)
                if ((currentDistributionSection.getDistributionType()
                    == DistributionSection.DATA_DISTRIBUTION
                    || currentDistributionSection.getDistributionType()
                    == DistributionSection.INLINE_DATA_DISTRIBUTION)
                ...
                if (currentDistributionSection.getAccessSection() != null) {
                    currentDistributionSection.getAccessSection()
                        .setDataFileName(currentDistributionSection.getDataFileName());
                }
        ...
        if (accessSection != null) {
            ...
            if (accessSectionObj.getReferences() == null) {
                ...
                for (int i = 0; i < accessObjectList.size(); i++) {
                    ...
                    if (permOrder.equals(AccessControlInterface.DENYFIRST) && ignoreDenyFirst) {
                        logMetacat.warn(
                            "Metacat no longer supports EML 'denyFirst' access rules - ignoring "
                                + "this access block, subtree id: "
                                + accessObjId);
                        continue;
                    }
                    if (referenceId != null && accessObj != null && referenceId.equals(
                        accessObjId)) {
                ...
                if (!findAccessObject) {
                    throw new SAXException(
                        "The referenceid: " + referenceId + " is not access subtree");
                }// if
            }// else
    ...
    private void compareNodeStacks(Stack<NodeRecord> stack1, Stack<NodeRecord> stack2)
        ...
        if (!stack2.isEmpty()) {
            logMetacat.error("stack2 still has some elements while stack " + "is empty! ");
            throw new SAXException(UPDATEACCESSERROR + identifier);
        }// if
    }// comparingNodeStacks
            ...
            if (distributionType == DistributionSection.DATA_DISTRIBUTION
                ...
                if (permOrder.equals(AccessControlInterface.DENYFIRST) && ignoreDenyFirst) {
                    logMetacat.warn(
                        "Metacat no longer supports EML 'denyFirst' access rules - ignoring this "
                            + "access block: "
                            + distributionSection.getDataFileName());
                    continue;
                }

                // We want to check file permissions for all online data updates and inserts, or
                // for
                // inline updates.
//                if (distributionType == DistributionSection.DATA_DISTRIBUTION
//                        || (distributionType == DistributionSection.INLINE_DATA_DISTRIBUTION &&
//                        action == "UPDATE")) {
                ...
                if (distributionType == DistributionSection.DATA_DISTRIBUTION) {
                    try {
                        ...
                        if (action == "UPDATE") {
                            ...
                            int latestRevision =
                                DBUtil.getLatestRevisionInDocumentTable(docidWithoutRev);
                            if (latestRevision > 0) {
                                previousDocid = docidWithoutRev + PropertyService.getProperty(
                                    "document.accNumSeparator") + latestRevision;
                            }
                        }
                        ...
                        PermissionController currentController =
                            new PermissionController(dataDocid);

                        if (AccessionNumber.accNumberUsed(docid) && !(
                            controller.hasPermission(user, groups, "WRITE")
                                || currentController.hasPermission(user, groups, "WRITE"))) {
                            throw new SAXException(
                                UPDATEACCESSERROR + identifier + " for the data object "
                                    + dataDocid);
                        }
                    } catch (SQLException sqle) {
                        throw new SAXException(
                            "Database error checking user permissions: " + sqle.getMessage());
                    } catch (Exception e) {
                        throw new SAXException(
                            "General error checking user permissions: " + e.getMessage());
                    }
                } else if (distributionType == DistributionSection.INLINE_DATA_DISTRIBUTION
                    && action == "UPDATE") {
                    try {
                        ...
                        if (!controller.hasPermission(user, groups, "WRITE")) {
                            throw new SAXException(
                                UPDATEACCESSERROR + identifier + " for an inline object");
                        }
                    } catch (SQLException sqle) {
                        throw new SAXException(
                            "Database error checking user permissions: " + sqle.getMessage());
     ...
     *
     */
    private void deleteAccessRule(AccessSection accessSection, boolean topLevel)
        throws SAXException {
        ...
        try {
            ...
            if (topLevel) {
                try {
                    guid =
                        IdentifierManager.getInstance().getGUID(docid, Integer.valueOf(revision));
    ...
    private void writeGivenAccessRuleIntoDB(
        AccessSection accessSection, boolean topLevel, String subSectionId) throws SAXException {
        if (accessSection == null) {
            throw new SAXException("The access object is null");
        }
        ...
        if (topLevel) {
            ...
            sql = "INSERT INTO xml_access (guid,principal_name, "
                + "permission, perm_type, perm_order, accessfileid, subtreeid" + ") VALUES"
                + " (?, ?, ?, ?, ?, ?, ?)";
        }
        try {
    ...
    private void deletePermissionsInAccessTable() throws SAXException {
        ...
        try {
            ...
            String sql = "DELETE FROM xml_access " +
                ...
                "OR guid IN " + "(SELECT xa.guid from xml_access xa, identifier id"
                + " WHERE xa.accessfileid = id.guid " + " AND id.docid = ?" + " AND id.rev = ?)";
    ...
    private void handleOnlineUrlDataFile(String url) throws SAXException {
        ...
        if (currentDistributionSection == null) {
            throw new SAXException(
                "Trying to set the online file name for a null" + " distribution section");
        }
        ...
        if (accessionNumber == null) {
            ...
            currentDistributionSection.setDistributionType(
                DistributionSection.ONLINE_DATA_DISTRIBUTION);
        } else {
            ...
            try {
                ...
                if (!writeAccessRules) {
                    logMetacat.warn(
                        "Not configured to write access rules for data referenced by: " + url);
                    return;
                }
                guid = docid + "." + rev;
                IdentifierManager.getInstance().createMapping(guid, guid);
            ...
            currentDistributionSection.setDistributionType(DistributionSection.DATA_DISTRIBUTION);
            ...
            try {
                if (!AccessionNumber.accNumberUsed(docid)) {
                    ...
                    // see: https://projects.ecoinformatics.org/ecoinfo/issues/5647
                    PermissionController previousController =
                        new PermissionController(previousDocid);
                    PermissionController currentController =
                        new PermissionController(accessionNumber);
                    if (writeAccessRules) {
                        if (previousController.hasPermission(
                            user, groups, AccessControlInterface.ALLSTRING)
                            || previousController.hasPermission(
                            user, groups, AccessControlInterface.CHMODSTRING)
                            || currentController.hasPermission(
                            user, groups, AccessControlInterface.ALLSTRING)
                            || currentController.hasPermission(
                            user, groups, AccessControlInterface.CHMODSTRING)) {
                            onlineDataFileIdInTopAccessVector.add(guid);
                ...
                logMetacat.error(
                    "Eorr in " + "Eml210SAXHanlder.handleOnlineUrlDataFile is " + e.getMessage());
                throw new SAXException(e.getMessage());
            }
        }
    }
