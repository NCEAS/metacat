 ...
 * Valid actions are:
 *
 * action=login username password qformat action=logout qformat action=query -- query the values of
 * all elements and attributes and return a result set of nodes meta_file_id -- returndoctype --
 * filterdoctype -- returnfield -- owner -- site -- operator -- casesensitive -- searchmode --
 * anyfield -- action=spatial_query -- run a spatial query.  these queries may include any of the
 * queries supported by the WFS / WMS standards xmax -- ymax -- xmin -- ymin -- skin -- pagesize --
 * pagestart -- action=squery -- structured query (see pathquery.dtd) query -- pagesize -- pagestart
 * -- action=export -- export a zip format for data packadge docid -- action=read -- read any
 * metadata/data file from Metacat and from Internet archiveEntryName -- docid -- qformat --
 * metadatadocid -- action=readinlinedata -- read inline data only inlinedataid action=insert --
 * insert an XML document into the database store qformat -- docid -- doctext -- dtdtext --
 * action=insertmultipart -- insert an xml document into the database using multipart encoding
 * qformat -- docid -- action=update -- update an XML document that is in the database store qformat
 * -- docid -- doctext -- dtdtext -- action=delete -- delete an XML document from the database store
 * docid -- action=validate -- validate the xml contained in valtext valtext -- docid --
 * action=setaccess -- change access permissions for a user on a document. docid -- principal --
 * permission -- permType -- permOrder -- action=getaccesscontrol -- retrieve acl info for Metacat
 * document docid -- action=getprincipals -- retrieve a list of principals in XML
 * action=getalldocids -- retrieves a list of all docids registered with the system scope --
 * action=getlastdocid -- scope -- username -- action=isregistered -- checks to see if the provided
 * docid is registered docid -- action=getrevisionanddoctype -- get a document's revision and
 * doctype from database docid -- action=getversion -- action=getdoctypes -- retrieve all doctypes
 * (publicID) action=getdtdschema -- retrieve a DTD or Schema file doctype -- action=getlog -- get a
 * report of events that have occurred in the system ipAddress --  filter on one or more IP
 * addresses> principal -- filter on one or more principals (LDAP DN syntax) docid -- filter on one
 * or more document identifiers (with revision) event -- filter on event type (e.g., read, insert,
 * update, delete) start -- filter out events before the start date-time end -- filter out events
 * before the end date-time action=getloggedinuserinfo -- get user info for the currently logged in
 * user ipAddress --  filter on one or more IP addresses> principal -- filter on one or more
 * principals (LDAP DN syntax) docid -- filter on one or more document identifiers (with revision)
 * event -- filter on event type (e.g., read, insert, update, delete) start -- filter out events
 * before the start date-time end -- filter out events before the end date-time action=shrink --
 * Shrink the database connection pool size if it has grown and extra connections are no longer
 * being used. action=buildindex -- docid -- action=refreshServices -- action=scheduleWorkflow --
 * Schedule a workflow to be run.  Scheduling a workflow registers it with the scheduling engine and
 * creates a row in the scheduled_job table.  Note that this may be extracted into a separate
 * servlet. delay -- The amount of time from now before the workflow should be run.  The delay can
 * be expressed in number of seconds, minutes, hours and days, for instance 30s, 2h, etc. starttime
 * -- The time that the workflow should first run.  If both are provided this takes precedence over
 * delay.  The time should be expressed as: MM/dd/yyyy HH:mm:ss with the timezone assumed to be that
 * of the OS. endtime -- The time when the workflow should end. The time should be expressed as:
 * MM/dd/yyyy HH:mm:ss with the timezone assumed to be that of the OS. intervalvalue -- The numeric
 * value of the interval between runs intervalunit -- The unit of the interval between runs.  Can be
 * s, m, h, d for seconds, minutes, hours and days respectively workflowid -- The lsid of the
 * workflow that we want to schedule.  This workflow must already exist in the database. karid --
 * The karid for the workflow that we want to schedule. workflowname -- The name of the workflow.
 * forwardto -- If provided, forward to this page when processing is done. qformat -- If provided,
 * render results using the stylesheets associated with this skin.  Default is xml.
 * action=unscheduleWorkflow -- Unschedule a workflow.  Unscheduling a workflow removes it from the
 * scheduling engine and changes the status in the scheduled_job table to " unscheduled.  Note that
 * this may be extracted into a separate servlet. workflowjobname -- The job ID for the workflow run
 * that we want to unschedule.  This is held in the database as scheduled_job.name forwardto -- If
 * provided, forward to this page when processing is done. qformat -- If provided, render results
 * using the stylesheets associated with this skin.  Default is xml. action=rescheduleWorkflow --
 * Unschedule a workflow.  Rescheduling a workflow registers it with the scheduling engine and
 * changes the status in the scheduled_job table to " scheduled.  Note that this may be extracted
 * into a separate servlet. workflowjobname -- The job ID for the workflow run that we want to
 * reschedule.  This is held in the database as scheduled_job.name forwardto -- If provided, forward
 * to this page when processing is done. qformat -- If provided, render results using the
 * stylesheets associated with this skin.  Default is xml. action=deleteScheduledWorkflow -- Delete
 * a workflow.  Deleting a workflow removes it from the scheduling engine and changes the status in
 * the scheduled_job table to " deleted.  Note that this may be extracted into a separate servlet.
 * workflowjobname -- The job ID for the workflow run that we want to delete.  This is held in the
 * database as scheduled_job.name forwardto -- If provided, forward to this page when processing is
 * done. qformat -- If provided, render results using the stylesheets associated with this skin.
 * Default is xml. action=reindex -- rebuild the solr index for the specified pids. pid -- the id of
 * the document which will be rebuilt slor index. action=reindexall -- rebuild the solr index for
 * all objects in the systemmetadata table.
 *
 * Here are some of the common parameters for actions doctype -- document type list returned by the
 * query (publicID) qformat=xml -- display resultset from query in XML qformat=html -- display
 * resultset from query in HTML qformat=zip -- zip resultset from query docid=34 -- display the
 * document with the document ID number 34 doctext -- XML text of the document to load into the
 * database acltext -- XML access text for a document to load into the database dtdtext -- XML DTD
 * text for a new DTD to load into Metacat XML Catalog query -- actual query text (to go with
 * 'action=query' or 'action=squery') valtext -- XML text to be validated scope --can limit the
 * query by the scope of the id docid --the docid to check datadoc -- data document name (id)
 */
public class MetaCatServlet extends HttpServlet {
    ...
    private static final Log logMetacat = LogFactory.getLog(MetaCatServlet.class);
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            logMetacat.debug(
                "MetaCatServlet.init - ServiceService singleton created " + serviceService);
            ...
            ServiceService.registerService(
                "SkinPropertyService", SkinPropertyService.getInstance());
            ...
            String errorMessage =
                "Service problem while initializing MetaCat Servlet: " + se.getMessage();
            logMetacat.error("MetaCatServlet.init - " + errorMessage);
            throw new ServletException(errorMessage);
        } catch (MetacatUtilException mue) {
            String errorMessage =
                "Metacat utility problem while initializing MetaCat Servlet: " + mue.getMessage();
            logMetacat.error("MetaCatServlet.init - " + errorMessage);
            throw new ServletException(errorMessage);
        }
    }
    ...
    /**
     * Initialize the remainder of the servlet. This is the part that can only be initialized after
     * metacat properties have been configured
     *
    ...
    public void initSecondHalf(ServletContext context) throws ServletException {
        try {
            ...
            // schema, which accidentally points to wrong version of eml-resource.xsd.
            String correctedEML201Doc =
                PropertyService.getProperty("document.eml201DocumentCorrected");
            if (correctedEML201Doc != null && correctedEML201Doc.equals(FALSE)) {
                logMetacat.info(
                    "MetaCatServlet.initSecondHalf - Start to correct eml201 documents");
                ...
                logMetacat.info(
                    "MetaCatServlet.initSecondHalf - Finish to correct eml201 documents");
            }
            ...
            int indexingTimerTaskTime =
                Integer.parseInt(PropertyService.getProperty("database.indexingTimerTaskTime"));
            int delayTime =
                Integer.parseInt(PropertyService.getProperty("database.indexingInitialDelay"));
            ...
            if (PropertyService.getProperty("spatial.runSpatialOption").equals("true")
                ...
                // entire spatial cache
                // this may be expensive with many documents
                SpatialHarvester sh = new SpatialHarvester();
                sh.regenerate();
                sh.destroy();

                // After running the first time, we want to set
                ...
                logMetacat.info(
                    "MetaCatServlet.initSecondHalf - Spatial Harvester Time  " + (after - before)
                        + "ms");

            } else {
                logMetacat.info(
                    "MetaCatServlet.initSecondHalf - Spatial cache is not set to regenerate on "
                        + "restart");
            }
            ...
            String replicationLogPath =
                PropertyService.getProperty("replication.logdir") + FileUtil.getFS()
                    + ReplicationService.REPLICATION_LOG_FILE_NAME;
            ...
            if (FileUtil.getFileStatus(replicationLogPath) < FileUtil.EXISTS_READ_WRITABLE) {
                logMetacat.error(
                    "MetaCatServlet.initSecondHalf - Replication log file: " + replicationLogPath
            ...
            logMetacat.warn(
                "MetaCatServlet.initSecondHalf - Metacat (" + MetacatVersion.getVersionID()
                    + ") initialized.");

        } catch (SQLException e) {
            String errorMessage =
                "SQL problem while initializing MetaCat Servlet: " + e.getMessage();
            logMetacat.error("MetaCatServlet.initSecondHalf - " + errorMessage);
            throw new ServletException(errorMessage);
        } catch (IOException ie) {
            String errorMessage =
                "IO problem while initializing MetaCat Servlet: " + ie.getMessage();
            logMetacat.error("MetaCatServlet.initSecondHalf - " + errorMessage);
            throw new ServletException(errorMessage);
            ...
            String errorMessage =
                "Utility problem while initializing MetaCat Servlet: " + ue.getMessage();
            logMetacat.error("MetaCatServlet.initSecondHalf - " + errorMessage);
            throw new ServletException(errorMessage);
        }
    }
    ...
    private void checkIndexPaths() {
        ...
        try {
            needCheck =
                Boolean.parseBoolean(PropertyService.getProperty("dbquery.init.check.newpath"));
        } catch (Exception e) {
            logMetacat.warn(
                "MetaCatServlet.checkIndexPaths - we can't get the property value of \"dbquery"
                    + ".init.check.newpath\" and the default value \"false\" will be used since "
                    + e.getMessage());
        }
        logMetacat.info("MetaCatServlet.checkIndexPaths - the final of needCheckInexPath value is "
                            + needCheck);
        if (!EnabledQueryEngines.getInstance().isEnabled(EnabledQueryEngines.PATHQUERYENGINE)
            || !needCheck) {
            logMetacat.info(
                "MetaCatServlet.checkIndexPaths - the pathquery is disabled or the property "
                    + "\"dbquery.init.check.newpath\" is set false, so it does nothing for "
                    + "checking path_index");
            return;
        }
        logMetacat.info(
            "MetaCatServlet.checkIndexPaths - after checking is the pathquery enabled or not...");
        ...
        if (pathsForIndexing != null && !pathsForIndexing.isEmpty()) {
            ...
            logMetacat.debug(
                "MetaCatServlet.checkIndexPaths - Indexing paths specified in metacat.properties."
                    + "...");
            ...
            for (String pathIndex : pathsForIndexing) {
                logMetacat.debug("MetaCatServlet.checkIndexPaths - Checking if '" + pathIndex
                                     + "' is indexed.... ");
                ...
                try {
                    ...
                    pstmt =
                        conn.prepareStatement("SELECT * FROM xml_path_index " + "WHERE path = ?");
                    ...
                    if (!rs.next()) {
                        ...
                        logMetacat.debug(
                            "MetaCatServlet.checkIndexPaths - Inserting following path in "
                                + "xml_path_index: "
                                + pathIndex);
                        if (pathIndex.indexOf("@") < 0) {
                            pstmt = conn.prepareStatement("SELECT DISTINCT n.docid, "
                                                              + "n.nodedata, n.nodedatanumerical,"
                                                              + " n.nodedatadate, n.parentnodeid"
                                                              + " FROM xml_nodes n, xml_index i "
                                                              + "WHERE"
                                                              + " i.path = ? and n.parentnodeid=i"
                                                              + ".nodeid and"
                                                              + " n.nodetype LIKE 'TEXT' order by"
                                                              + " n.parentnodeid");
                        } else {
                            pstmt = conn.prepareStatement("SELECT DISTINCT n.docid, "
                                                              + "n.nodedata, n.nodedatanumerical,"
                                                              ...
                                                              + " FROM xml_nodes n, xml_index i "
                                                              + "WHERE"
                                                              + " i.path = ? and n.nodeid=i"
                                                              + ".nodeid and"
                                                              + " n.nodetype LIKE 'ATTRIBUTE' "
                        ...
                        int count = 0;
                        logMetacat.debug(
                            "MetaCatServlet.checkIndexPaths - Executed the select statement for: "
                                + pathIndex);
                        ...
                        try {
                            ...
                            logMetacat.error(
                                "MetaCatServlet.checkIndexPaths - Exception:" + e.getMessage());
                            e.printStackTrace();
                        }
                        ...
                        logMetacat.info("MetaCatServlet.checkIndexPaths - Indexed " + count
                                            + " records from xml_nodes for '" + pathIndex + "'");
                    ...
                    logMetacat.error(
                        "MetaCatServlet.checkIndexPaths - Error in MetaCatServlet.checkIndexPaths: "
                            + e.getMessage());
                } finally {
    ...
    private void handleGetOrPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (requestEncoding == null) {
            logMetacat.debug(
                "null requestEncoding, setting to application default: " + DEFAULT_ENCODING);
        ...
        if (!ConfigurationUtil.isMetacatConfigured()) {
            try {
                ...
                logMetacat.error(
                    "MetacatServlet.handleGetOrPost - utility error when forwarding to "
        ...
        logMetacat.info(
            "MetacatServlet.handleGetOrPost - the _fullyInitialized value is " + _fullyInitialized);
        ...
        try {
            @SuppressWarnings("unused") DBConnectionPool pool = DBConnectionPool.getInstance();
//            pool.printMethodNameHavingBusyDBConnection();
        } catch (SQLException e) {
            logMetacat.error(
                "MetaCatServlet.handleGetOrPost - Error in MetacatServlet.handleGetOrPost: "
                    + e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            ...
            if (pathInfo != null) {
                ...
                if (path.length > 1) {
                    ...
                    docs[0] = docidToRead;
                    logMetacat.debug(
                        "MetaCatServlet.handleGetOrPost - READING DOCID FROM PATHINFO: " + docs[0]);
                        ...
                        if (docidNoRev != null) {
                            ...
                            try {
                                ...
                                response.sendRedirect(
                                    SystemUtil.getServerURL() + "/" + uiContext + "/#view/" + pid);
                                return;
                            } catch (McdbDocNotFoundException nfe) {
                                logMetacat.warn(
                                    "Could not locate PID for docid: " + docidToRead, nfe);
                            }
                        } else {
                            ...
                            out.println("<error>");
                            out.println("The docid " + docidToRead
                                            + " doesn't match the format and doesn't exist.");
                            out.println("</error>");
                            out.close();
                            return;
                        }
            ...
            if (action.equals("login")) {
                ...
                if (token != null && !token.equals("")) {
                    Writer out =
                        new OutputStreamWriter(response.getOutputStream(), DEFAULT_ENCODING);
            ...
            if (action.equals("query")) {
                ...
                logMetacat.debug(
                    "MetaCatServlet.handleGetOrPost - ******************* SPATIAL QUERY "
                        + "********************");
                Writer out = new OutputStreamWriter(response.getOutputStream(), DEFAULT_ENCODING);
                ...
                if (params.get("archiveEntryName") != null) {
                    ArchiveHandler.getInstance()
                        .readArchiveEntry(params, request, response, userName, password,
                                          groupNames);
                } else {
                ...
                if ((userName != null) && !userName.equals("public")) {
                    ...
                    File file = null;
                    handler.handleInsertOrUpdateAction(request.getRemoteAddr(),
                                                       request.getHeader("User-Agent"), response,
                                                       out, params, userName, groupNames, true,
                                                       true, null, formatId, checksum, file);
                } else {
                    response.setContentType("text/xml");
                    out.println("<?xml version=\"1.0\"?>");
                    out.println("<error>");
                    String cleanMessage = StringEscapeUtils.escapeXml(
                        "Permission denied for user " + userName + " " + action);
                ...
                if ((userName != null) && !userName.equals("public")) {
                    ...
                    out.println("<error>");
                    String cleanMessage =
                        StringEscapeUtils.escapeXml("Permission denied for " + action);
                    out.println(cleanMessage);
                    out.println("</error>");
                }
                out.close();
                ...
                handler.handleGetLogAction(
                    params, request, response, userName, groupNames, sessionId);
                ...
                PrintWriter out = response.getWriter();
                if (!AuthUtil.isAdministrator(userName, groupNames)) {
                    out.println("The user " + userName
                                    + " is not the administrator of the Metacat and doesn't have "
                                    + "the permission to call the method.");
                    out.close();
                    return;
                }
                ...
                MetacatHandlerPlugin handlerPlugin =
                    MetacatHandlerPluginManager.getInstance().getHandler(action);
                if (handlerPlugin != null) {
                    if (isReadOnly(response)) {
                        return;
                    }
                    handlerPlugin.handleAction(
                        action, params, request, response, userName, groupNames, sessionId);
                } else {
                    PrintWriter out = response.getWriter();
                    out.println("<?xml version=\"1.0\"?>");
                    out.println("<error>");
                    String cleanMessage = StringEscapeUtils.escapeXml(
                        "Error: action: " + action + " not registered.  Please report this error.");
                    out.println(cleanMessage);
                    out.println("</error>");
                    out.close();
                }
            ...
            // written to output.  Continue on and let it display.
        }
    }
    ...
    public static boolean isReadOnly(HttpServletResponse response) throws IOException {
        ...
        if (readOnly) {
            ...
            out.println("<?xml version=\"1.0\"?>");
            out.println("<error>");
            out.println("The Metacat is in read-only mode and your request can't be fulfilled. "
    ...
    public static void initializeSitemapTask(MetacatHandler handler) {
        ...
        try {
            Boolean.parseBoolean(PropertyService.getProperty("sitemap.enabled"));
        } catch (PropertyNotFoundException pnfe) {
